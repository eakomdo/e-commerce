...
class RegistrationView(APIView):
    ...
    def post(self, request):
        ...
        if serializer.is_valid(raise_exception=True):
            ...
            verification_link = (
                f"http://localhost:8000/api/users/verify-email/?uid={uid}&token={token}"
            )
...
class VerifyEmailView(APIView):
    ...
    def get(self, request):
        ...
        if not uid or not token:
            return Response(
                {"message": "Missing uid or token"}, status=status.HTTP_400_BAD_REQUEST
            )
        ...
        if not default_token_generator.check_token(user, token):
            return Response(
                {"message": "Invalid link or url has expired"},
...
class PasswordConfirmView(APIView):
    ...
    def post(self, request):
        ...
        if serializer.is_valid(raise_exception=True):
            ...
            if not default_token_generator.check_token(user, token):
                return Response(
                    {"message": "Invalid link or token has expired"},
                    status=status.HTTP_400_BAD_REQUEST
                )
